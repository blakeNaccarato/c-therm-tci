...
- Remove the cap from the sensor, shown in {numref}`00-06-capped`. Please remember to [](#protect-the-sensor)!
...
The active test indicator will read "reading sample" as in {numref}`15-measuring` when a measurement is being taken. Wait for a few measurements to finish before leaving the instrument unattended.
...
Follow the process below to export an XML file of all tests. It is easier to export and process all tests than to just process the latest ones, as the data process includes an Excel file which takes averages of measurements grouped by Project, Material Group, Material, and Lot Number.
...
You can search for test results using the "Open Test" dialog. Note that the `*` character functions as a wildcard when searching.
...
The test category hierarchy is shown below.
...
You can create test methods and vary the delay between measurements.
...
Sample categories connect to one another, allowing you to browse the Materials belonging to a Material Group, the Lots for each Material, and so-on.
